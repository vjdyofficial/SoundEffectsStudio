...
audioCanvas.height = 512;

audioCanvasPreview.width = 512;
audioCanvasPreview.height = 512;
...
function updateDB(dataArray) {
    ...
    document.getElementById('info_dBArray').textContent = `${(avgDB - 30).toFixed(1)} dB`;
}
...
ipcRenderer.on('updatearray', event => {
    analyserL.getByteTimeDomainData(dataL);
    analyserR.getByteTimeDomainData(dataR);
    analysermeter.getByteTimeDomainData(dataArrayMeter);
    analysermeter.getByteFrequencyData(freqData2);
...
function updateMeter() {
    ...
    ipcRenderer.send('send-peakscale', levelL, levelR)
...
setInterval(() => {
    if (canUpdate <= 300) {
        document.getElementById('info_dbactive').textContent = 'Active';
        if (shouldSendFrame()) {
            if (!toggleExternal) {
        ...
        document.getElementById('info_dbactive').textContent = 'Inactive';
    }

    if (total == 0 && canUpdate <= 300) {
        canUpdate++
    } else if (!total == 0) {
        canUpdate = 0;
    }
...
setInterval(() => {
    updateDB(freqData2);
    document.getElementById('info_totalarray').textContent = total + " out of 32640" + " buffers";
}, 150);