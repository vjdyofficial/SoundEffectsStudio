...
const { getEmbeddedLyrics } = require("./getEmbeddedLyrics");
            ...
            if (types.some(type => file.type.startsWith(type.split('/')[0]))) {
                ...
                openAndLoadFile(file).finally(() => {
                    ...
                    if (currentMediaEl.duration <= 7200) {
                        StartWaveform(file, `audioWaveTime_MediaExtDeck${assignedDeck}`)
                            .then(() => console.log('Waveform drawn!'))
        ...
        if (file) {
            ...
            if (types.some(type => file.type.startsWith(type.split('/')[0]))) {
                ...
                openAndLoadFile(file).finally(() => {
                    hiddenInput.value = ""
                    if (currentMediaEl.duration <= 7200) {
                        StartWaveform(file, `audioWaveTime_media${deckId}`)
                            .then(() => console.log('Waveform drawn!'))
                            .catch(err => console.error(err))
                    } else {
                        snackbar("Audio duration exceeds 2 hours, skipping waveform generation.");
