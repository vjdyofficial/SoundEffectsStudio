...
function PitchShift(audioContext){
  ...
  // --- Smooth wrapper for original transpose ---
  let currentTranspose = 0
  let targetTranspose = 0
  const smoothingFactor = 0.1

  Object.defineProperty(node, 'smoothTranspose', {
    set: function(value){
      targetTranspose = value
      requestAnimationFrame(rampStep)
    },
    get: function(){
      return targetTranspose
    }
  })

  function rampStep(){
    if(Math.abs(currentTranspose - targetTranspose) < 0.01){
      currentTranspose = targetTranspose
      node.transpose = currentTranspose
      return
    }
    currentTranspose += (targetTranspose - currentTranspose) * smoothingFactor
    node.transpose = currentTranspose
    requestAnimationFrame(rampStep)
  }
...
Jungle.prototype.setPitchOffset = function(mult) {
    ...
    const rampTime = 0.15; // seconds for smoothing
