// Browser replacement for custom-audio-node
function createAudioNode(input, output, params = {}) {
  const node = output
  ...
  // Attach parameters (dry, wet)
  Object.keys(params).forEach(name => {
    const cfg = params[name]
    const target = cfg.target

    if (cfg.defaultValue !== undefined) {
      target.value = cfg.defaultValue
    }
    ...
    Object.defineProperty(node, name, {
      ...
      set(v) {
        target.value = Math.max(cfg.min ?? -Infinity, v)
      }
    })
  })
...
function PitchShift(audioContext) {
  ...
  // --- Original transpose property fixed ---
  var transpose = 0
  var ramp = 0.05  // slightly smoother default

  Object.defineProperty(node, 'transpose', {
    enumerable: true,   // now shows in console
    configurable: true,
    set: function (value) {
      transpose = getMultiplier(value)  // keep as number
...
window.PitchShift.transpose = PitchShift.prototype.transpose
...
function createFadeBuffer(context, activeTime, fadeTime) {
  var length1 = activeTime * 48000;
  var length2 = (activeTime - 2 * fadeTime) * 48000;
  var length = length1 + length2;
  var buffer = context.createBuffer(1, length, 48000);
  var p = buffer.getChannelData(0);

  var fadeLength = fadeTime * 48000;
...
function createDelayTimeBuffer(context, activeTime, fadeTime, shiftUp) {
  ...
  for (var i = 0; i < length1; ++i) {
    if (shiftUp)
      ...
      p[i] = (length1 - 2 * fadeTime - i) / length1;
    else
